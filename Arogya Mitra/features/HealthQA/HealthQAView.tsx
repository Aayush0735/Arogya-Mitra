        ...
        setChatMessages([
            {
                ...
                sender: 'ai',
                text: "Hello! I'm आरोग्य Mitra, your AI health assistant. How can I help you today? Feel free to ask me any health-related questions.",
                timestamp: new Date(),
            }
        ]);
    }
    ...
    return (
      <div className="p-3 sm:p-4">
        <DisclaimerBox 
          ...
          message="The Gemini API key is not configured for आरोग्य Mitra. The Health Q&A assistant requires a valid API key to function. Please ensure the API_KEY environment variable is set by the hosting environment."
    ...
    <div className="flex flex-col h-full bg-white rounded-xl shadow-xl overflow-hidden border border-neutral-light">
        ...
        {isLoading && (
          <div className="flex justify-start py-2">
             <div className="flex items-center space-x-2 bg-white text-neutral-darkest border border-neutral-light rounded-2xl rounded-bl-none p-3 shadow-md">
                <LoadingSpinner /> 
                <p className="text-sm text-neutral-medium">आरोग्य Mitra is thinking...</p>
            </div>
          </div>
        )}
         {error && !isLoading && ( 
      ...
      <form onSubmit={handleSubmit} className="p-3 sm:p-4 border-t border-neutral-light bg-neutral-lightest">
        <div className="flex items-center space-x-2 sm:space-x-3">
         ...
         <p className="text-xs text-neutral-DEFAULT mt-2 pl-1">आरोग्य Mitra may provide informative responses but it's not a substitute for medical advice.</p>
      </form>
    </div>
  );
};
